...
impl Tail {
    pub fn new_tail(index: u32, g_dash: &PointG2, gamma: &GroupOrderElement) -> Result<Tail, IndyCryptoError> {
...
impl RevocationTailsGenerator {
    ...
    pub fn get_gamma(&self) ->&GroupOrderElement{
	&self.gamma
    }
    pub fn get_g_dash(&self) ->&PointG2{
	&self.g_dash
    }
	   
    pub fn generate_tail(&self,index: u32) -> Result<Option<Tail>, IndyCryptoError>{
	let tail = Tail::new_tail(index, &self.g_dash, &self.gamma)?;
	Ok(Some(tail))
     }
